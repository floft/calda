    ...
    for modality, modality_data in enumerate(data):
        ...
        for i in range(num_features):
            ...
            if modality_feature_names is not None:
                assert len(modality_feature_names) > i, \
                    "not enough features, looking for feature " + str(i) \
                    + " (zero indexed), but only " \
                    + str(len(modality_feature_names)) \
                    + " features: " + str(modality_feature_names)
